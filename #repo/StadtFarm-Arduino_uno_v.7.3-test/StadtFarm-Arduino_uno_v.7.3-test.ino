...
- check sensors, if one (RTC, Light, ..) is not working, go into emergency operation mode ("main operation"), than switch "Sensor_Error = true
...
unsigned long runnningPump = MILISEC_PER_MIN * 2.5; // running Timer for pumps in Min. -> security Timer if capacity sensor doesnt work
...
//  pumpManagement();  // replaced by Pump_Moistusture_Management
//  LightSensorReboot();
...
/*
//////////// Pump Switch Modules ////////////
// for pump management by timer, now replaced by Pump_Moistusture_Management
void pumpManagement() {
  digitalWrite(pumpSwitch_1, pumpState);
