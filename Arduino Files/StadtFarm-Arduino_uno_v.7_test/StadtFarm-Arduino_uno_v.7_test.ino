...
int CD_Hours = 0;
int CD_Minutes = 10;  // hytseresis timer to see how long it takes until waterpump starts or maxwater level is reached after capacity value exceeded
int CD_Seconds = 0;
//bool CDbool = true;
//uint32_t start, stop;
...
bool day_time = true;
bool switch_light_on = true;  // for additonal button to switch light on (maintenance ore else)
...
const int Capac_WaterValue_Min = 550;  // Dry, Limit moisture, start pump
const int Capac_WaterValue_Full = 340;  // Limit for max Water level for filling
  ...
  void Pump_Moistusture_Management(){
    ...
    digitalWrite(pumpSwitch_1, pumpState);
      if (SoilMoistureValue <= Capac_WaterValue_Full){
        pumpState = LOW;
      }else if(SoilMoistureValue >= Capac_WaterValue_Min){
        
      }
      if (SoilMoistureValue >= Capac_WaterValue_Min){  // - hysteresis_Capac_Sensor){
        pumpState = HIGH;
     } else{
        pumpState = LOW;
 }
...
// Moisture and Pump state
    ...
    Serial.print("%RH, ");
    if (Capac_WaterValue_Full <= SoilMoistureValue <= Capac_WaterValue_Min){
